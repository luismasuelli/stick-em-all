...
const economyContractABI = [
    {
        "inputs": [
            {
                "internalType": "address",
                "name": "_worldsManagement",
                "type": "address"
            }
        ],
        "stateMutability": "nonpayable",
 ...
 */
export default async function mainContractClients(web3, account) {
    const mainContractAddress = process.env.REACT_APP_MAIN_CONTRACT;
    if (mainContractAddress && web3.utils.isAddress(mainContractAddress) && (
        web3.utils.toChecksumAddress(mainContractAddress) !== web3.utils.toChecksumAddress(ZERO_ADDRESS)
    )) {
        let main = new web3.eth.Contract(
            mainContractABI, mainContractAddress, {from: account}
        );
        let economyContractAddress = await main.methods.economy().call();
        let economy = new web3.eth.Contract(
            economyContractABI, economyContractAddress
        );
        let worldsManagementContractAddress = await main.methods.worldsManagement().call();
        let worldsManagement = new web3.eth.Contract(
            worldsManagementContractABI, worldsManagementContractAddress
        );
